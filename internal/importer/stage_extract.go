...
func (pipe *pipeline) extract(ctx context.Context, in <-chan *pipelineItem, out chan<- *pipelineItem) error {
	for item := range in {
		if !item.isSidecar && !item.rejected && (item.verdict == actionNew || item.verdict == actionReimport) {
			extractedMetadata, err := pipe.importer.extractMetadata(pipe.fsys, item.scanned)
...
// the same guard.
func (imp *Importer) metadataFor(fsys fs.FS, scanned scannedFile, verdict action) metadata.Metadata {
	if verdict != actionNew && verdict != actionReimport {
		return metadata.Metadata{}
	}
	extractedMetadata, _ := imp.extractMetadata(fsys, scanned)
...
func (imp *Importer) extractMetadata(fsys fs.FS, scanned scannedFile) (metadata.Metadata, error) {
	if scanned.handler.Metadata == nil {
		return metadata.Metadata{}, nil
	}
	reader, closeReader, err := openSeeker(fsys, scanned.relPath)
	if err != nil {
		imp.Log.Warn("metadata: open failed", "path", scanned.relPath, "err", err)
	...
	if err != nil {
		...
		imp.Log.Warn("metadata extraction failed", "path", scanned.relPath, "err", err)
	}
	width, height := 0, 0
	...
	imp.Log.Debug("metadata extracted", "path", scanned.relPath,
