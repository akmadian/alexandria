...
- Five-stage pipeline: SCAN -> HASH -> MATCH -> EXTRACT -> WRITE (D25/D28: thumbnails and all signals are post-ingest enrichment jobs, never pipeline stages)
...
- Assets appear in grid incrementally as batches commit, with honest per-cell enrichment state (enriching / ready / failed — D25); EXTRACT yields dimensions/orientation so placeholder cells are correctly shaped
- Import summary at completion: N added, N updated, N skipped, N errors with file path and reason

### Thumbnail Generation

- Generated by the post-ingest enrichment engine (first enrichment job kind, D25/D28; viewport-priority ordering), stored in app data directory keyed by asset ID
...
### Signals Milestone (Enrichment Jobs)

*Architecture locked 2026-07-07 (C11: models propose as data, the user disposes via the query); engine design closed 2026-07-12 (D28 — the enrichment engine record). Every signal is a metadata column exposed as a query token type — no verdicts, no opaque scores driving hidden behavior. UX: `epics/frontend-culling-signals.md`. Subsumes the former standalone "Perceptual Hash" and "Technical Quality Scoring" entries.*

- **Cheap signals are fast enrichment jobs** (D25/D28): sharpness (Laplacian variance), highlight/shadow clipping % (histogram), phash — each reads the thumbnail artifact, ordered right behind thumbnail generation, so signals are effectively there when the user sits down to cull
