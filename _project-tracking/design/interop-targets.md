...
architecture; see `_project-tracking/backend/02-decision-log.md` (D6 registry
...
(see `backend/04-open-questions.md` #11), and landed on the feature that was
...
|---|---|---|---|
| **Premiere** | ✅ reads embedded XMP it wrote | ⚠️ requires container mutation | `xmpDM:Tracks`/`markers`, embedded only. Will **not** read a sidecar when the container supports embedding. |
| **Resolve** | ⚠️ user exports CSV | ✅ CSV (clip metadata) / EDL (markers) / FCPXML | "Resolve doesn't interpret XMP" — no sidecar, no embedded, no writeback to source. |
| **Final Cut Pro** | ⚠️ user exports FCPXML | ✅ FCPXML | Never writes metadata to media files. Everything lives in the `.fcpbundle` library DB. |
| **RAW stills** | ✅ | ✅ | `.xmp` sidecar; already shipped (D15/impl/06). |
| **PSD / AI / INDD / SVG** | ✅ | ✅ | Embedded XMP, no sidecar needed. Not yet exercised. |
| **Fonts / LUTs** | ❌ | ❌ | No layer exists. Say so. |
