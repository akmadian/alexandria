...
**Areas:** backend, seam. **Blocked by:** nothing. Created 2026-07-08 (backend audit) to give a
home to the work that keeps deferring to "the app-host milestone"; needs its design round, which
closes by minting this epic's tasks.
...
1. **Wails v2 wiring / composition root** — *split 2026-07-09; the composition root shipped with the seam round.* The composition root exists: `main.go` (thin entrypoint) + `app.go` (the
...
3. **Startup checks — named by the 2026-07-08 audit as ownerless, now owned here:**
   ...
   - **Backup-before-migration** — the P0 floor: one `VACUUM INTO` (never raw file copy) before
     ...
     separate design task — `../ideation/backend-open-questions.md` #16.
...
- The catalog backup system proper — design task, `../ideation/backend-open-questions.md` #16.
