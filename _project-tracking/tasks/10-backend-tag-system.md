# 10 — Tag system: the tag-UI backend

**Areas:** backend, seam. **Blocked by:** nothing (build when the tag-management UI is the
caller — the frontend rebuild reaching tag editing is the natural trigger).
**References:** D8 (classification), D22, `docs/data-model.md` §1, FR "Tags" / "Tag Colors"
(P0/P2), `internal/domain/tag.go`, `internal/sqlite/tag_repo.go` (the shipped consumer slice:
`Slugify`/`EnsureTag`/`AddAssetTags`/`ImportKeywords`/`RebuildTagPaths`, all green under
`-race`; the XMP keyword-union wiring lives in `internal/xmp`).

**Remaining scope** — the methods deliberately carved out until a UI calls them: `Tree`, `Get`,
`Update` (rename + the reparent path-rewrite specified below), `Delete`, a replace-semantics
`SetAssetTags` for the user-editing path, the color-`resolve` algorithm, and the seam methods
that expose them (`tagTree`/`createTag`/`updateTag`/`deleteTag`/`setAssetTags` — DEFERRED §7
row). FTS⋈tags remains its own design pass (section below). The sections that follow are the
design those methods build on — data model and semantics that already exist in the schema and
consumer slice are explained here so this task is self-contained; **on completion, fold the
durable sections (data model, performance rules, color inheritance, tombstone semantics) into
`docs/data-model.md` before deleting this file.**
