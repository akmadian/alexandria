...
→ **Start at `docs/v2/project-tracking/backend/00-START-HERE.md`**
...
keybindings DB table (dropped in v2 — now `keybindings.json`, impl/11), the standalone app-maintained
FTS design (v2: trigger-maintained), `sources.status` (v2: split into `enabled` + `connectivity`),
localStorage routing for UI persistence (v2: catalog-scoped `settings.json`, impl/11 — no DB table),
and the sequential importer design (v2: concurrent pipeline, spec in `v2/.../impl/04-ingest-pipeline.md`).
